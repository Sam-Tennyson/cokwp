...
function getEnvironment() {
  const envVar = (process.env.CASHFREE_ENV || process.env.NEXT_PUBLIC_CASHFREE_ENV || "TEST").toUpperCase();
  return envVar === "LIVE" ? CFEnvironment.PRODUCTION : CFEnvironment.SANDBOX;
}

function ensureClient() {
  if (client) return client;
  const isLive = (process.env.CASHFREE_ENV || process.env.NEXT_PUBLIC_CASHFREE_ENV || "TEST").toUpperCase() === "LIVE";
  const appId = (
    isLive
