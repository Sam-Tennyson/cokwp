      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
        {quizzes.map((quiz) => (
          ...
          >
            ...
            <button onClick={() => router.push(`/quiz/${quiz.id}`)} className='px-4 py-2 w-full bg-pink-500 text-white rounded-full font-bold shadow-sm hover:bg-pink-600 transition-colors'>Play</button>
          </div>
        ))}
      </div>
    </div>
